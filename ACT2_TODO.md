...
- **Elevation / mountains are fixed in code.** Act 2 now shares the Act-1 terrain-settle architecture:
  ...
  and all eight landmark mesas have matching hard blockers.
...
shares zone-aware terrain sampling and terrain-settling across zones, keeps the visual rim beyond the
playable radius (`walkR`), and makes mid-map mesas separate collidable scenery instead of walkable ground.
...
2. Final approach (current code): Act 2 now follows Act 1's open-world split. `buildSunscarTerrain()`
   ...
   Players/enemies ride `groundY` for dunes only, then blockers/clamps decide what is impassable.
